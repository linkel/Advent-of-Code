...
def part1(lines):
    ...
    coord_id_to_point = {coord_id: point for coord_id, point in enumerate(coordinates, start=1)}
...
def part2(lines, manhattan_limit=10000):
    ...
    for i in range(max_r + 1):
        for j in range(max_c + 1):
            size_shared_region += int(sum(abs(r - i) + abs(c - j) for r, c in coordinates) < manhattan_limit)
